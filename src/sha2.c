/*
---------------------sha2.c---------------------
Author :      Elerias
Date :        18.04.2021
Version :     0.8
Description : Implementation of SHA-224, SHA-256
SHA-384, SHA-512, SHA-512/224, SHA-512/256
------------------------------------------------

Description of SHA-2 : https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.180-4.pdf
*/

#include <stdio.h>
#include <stdint.h>
#include "sha2.h"
#include "global.h"

#define P32_1(x) (ROT32R2(x) ^ ROT32R13(x) ^ ROT32L10(x))
#define P32_2(x) (ROT32R6(x) ^ ROT32R11(x) ^ ROT32L7(x))
#define P32_3(x) (ROT32R7(x) ^ ROT32L14(x) ^ ((x) >> 3))
#define P32_4(x) (ROT32L15(x) ^ ROT32L13(x) ^ ((x) >> 10))

#define P64_1(x) (ROT64R28(x) ^ ROT64L30(x) ^ ROT64L25(x))
#define P64_2(x) (ROT64R14(x) ^ ROT64R18(x) ^ ROT64L23(x))
#define P64_3(x) (ROT64R1(x) ^ ROT64R8(x) ^ ((x) >> 7))
#define P64_4(x) (ROT64R19(x) ^ ROT64L3(x) ^ ((x) >> 6))

const uint32_t SHA2x32_K[64] =
{0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
 0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
 0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
 0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
 0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
 0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
 0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2};

const uint64_t SHA2x64_K[80] =
{0x428a2f98d728ae22, 0x7137449123ef65cd, 0xb5c0fbcfec4d3b2f, 0xe9b5dba58189dbbc,
 0x3956c25bf348b538, 0x59f111f1b605d019, 0x923f82a4af194f9b, 0xab1c5ed5da6d8118,
 0xd807aa98a3030242, 0x12835b0145706fbe, 0x243185be4ee4b28c, 0x550c7dc3d5ffb4e2,
 0x72be5d74f27b896f, 0x80deb1fe3b1696b1, 0x9bdc06a725c71235, 0xc19bf174cf692694,
 0xe49b69c19ef14ad2, 0xefbe4786384f25e3, 0x0fc19dc68b8cd5b5, 0x240ca1cc77ac9c65,
 0x2de92c6f592b0275, 0x4a7484aa6ea6e483, 0x5cb0a9dcbd41fbd4, 0x76f988da831153b5,
 0x983e5152ee66dfab, 0xa831c66d2db43210, 0xb00327c898fb213f, 0xbf597fc7beef0ee4,
 0xc6e00bf33da88fc2, 0xd5a79147930aa725, 0x06ca6351e003826f, 0x142929670a0e6e70,
 0x27b70a8546d22ffc, 0x2e1b21385c26c926, 0x4d2c6dfc5ac42aed, 0x53380d139d95b3df,
 0x650a73548baf63de, 0x766a0abb3c77b2a8, 0x81c2c92e47edaee6, 0x92722c851482353b,
 0xa2bfe8a14cf10364, 0xa81a664bbc423001, 0xc24b8b70d0f89791, 0xc76c51a30654be30,
 0xd192e819d6ef5218, 0xd69906245565a910, 0xf40e35855771202a, 0x106aa07032bbd1b8,
 0x19a4c116b8d2d0c8, 0x1e376c085141ab53, 0x2748774cdf8eeb99, 0x34b0bcb5e19b48a8,
 0x391c0cb3c5c95a63, 0x4ed8aa4ae3418acb, 0x5b9cca4f7763e373, 0x682e6ff3d6b2b8a3,
 0x748f82ee5defb2fc, 0x78a5636f43172f60, 0x84c87814a1f0ab72, 0x8cc702081a6439ec,
 0x90befffa23631e28, 0xa4506cebde82bde9, 0xbef9a3f7b2c67915, 0xc67178f2e372532b,
 0xca273eceea26619c, 0xd186b8c721c0c207, 0xeada7dd6cde0eb1e, 0xf57d4f7fee6ed178,
 0x06f067aa72176fba, 0x0a637dc5a2c898a6, 0x113f9804bef90dae, 0x1b710b35131c471b,
 0x28db77f523047d84, 0x32caab7b40c72493, 0x3c9ebe0a15c9bebc, 0x431d67c49c100d4c,
 0x4cc5d4becb3e42b6, 0x597f299cfc657e2a, 0x5fcb6fab3ad6faec, 0x6c44198c4a475817};

void SHA224Init(SHA2x32Context* context)
{
    context->H[0] = 0xc1059ed8;
    context->H[1] = 0x367cd507;
    context->H[2] = 0x3070dd17;
    context->H[3] = 0xf70e5939;
    context->H[4] = 0xffc00b31;
    context->H[5] = 0x68581511;
    context->H[6] = 0x64f98fa7;
    context->H[7] = 0xbefa4fa4;
    context->bitLen = 0;
    context->alg = 224;
}

void SHA256Init(SHA2x32Context* context)
{
    context->H[0] = 0x6a09e667;
    context->H[1] = 0xbb67ae85;
    context->H[2] = 0x3c6ef372;
    context->H[3] = 0xa54ff53a;
    context->H[4] = 0x510e527f;
    context->H[5] = 0x9b05688c;
    context->H[6] = 0x1f83d9ab;
    context->H[7] = 0x5be0cd19;
    context->bitLen = 0;
    context->alg = 256;
}

void SHA384Init(SHA2x64Context* context)
{
    context->H[0] = 0xcbbb9d5dc1059ed8;
    context->H[1] = 0x629a292a367cd507;
    context->H[2] = 0x9159015a3070dd17;
    context->H[3] = 0x152fecd8f70e5939;
    context->H[4] = 0x67332667ffc00b31;
    context->H[5] = 0x8eb44a8768581511;
    context->H[6] = 0xdb0c2e0d64f98fa7;
    context->H[7] = 0x47b5481dbefa4fa4;
    context->bitLen = 0;
    context->alg = 384;
}

void SHA512Init(SHA2x64Context* context)
{
    context->H[0] = 0x6a09e667f3bcc908;
    context->H[1] = 0xbb67ae8584caa73b;
    context->H[2] = 0x3c6ef372fe94f82b;
    context->H[3] = 0xa54ff53a5f1d36f1;
    context->H[4] = 0x510e527fade682d1;
    context->H[5] = 0x9b05688c2b3e6c1f;
    context->H[6] = 0x1f83d9abfb41bd6b;
    context->H[7] = 0x5be0cd19137e2179;
    context->bitLen = 0;
    context->alg = 512;
}

void SHA512_224Init(SHA2x64Context* context)
{
    context->H[0] = 0x8c3d37c819544da2;
    context->H[1] = 0x73e1996689dcd4d6;
    context->H[2] = 0x1dfab7ae32ff9c82;
    context->H[3] = 0x679dd514582f9fcf;
    context->H[4] = 0x0f6d2b697bd44da8;
    context->H[5] = 0x77e36f7304c48942;
    context->H[6] = 0x3f9d85a86a1d36c8;
    context->H[7] = 0x1112e6ad91d692a1;
    context->bitLen = 0;
    context->alg = 224;
}

void SHA512_256Init(SHA2x64Context* context)
{
    context->H[0] = 0x22312194fc2bf72c;
    context->H[1] = 0x9f555fa3c84c64c2;
    context->H[2] = 0x2393b86b6f53b151;
    context->H[3] = 0x963877195940eabd;
    context->H[4] = 0x96283ee2a88effe3;
    context->H[5] = 0xbe5e1e2553863992;
    context->H[6] = 0x2b0199fc2c85b8aa;
    context->H[7] = 0x0eb72ddc81c52ca2;
    context->bitLen = 0;
    context->alg = 256;
}

void SHA2x32ProcessBlock(SHA2x32Context* context, const unsigned char* block)
{
    context->bitLen += 512;

    uint32_t X[64];
    for (int k=0 ; k<16 ; k++)
    {
        ENCODE32BI(X[k], block[k*4], block[k*4+1], block[k*4+2], block[k*4+3])
    }
    for (int k=16 ; k<64 ; k++)
    {
        X[k] = P32_4(X[k-2]) + X[k-7] + P32_3(X[k-15]) + X[k-16];
    }

    uint32_t a = context->H[0];
    uint32_t b = context->H[1];
    uint32_t c = context->H[2];
    uint32_t d = context->H[3];
    uint32_t e = context->H[4];
    uint32_t f = context->H[5];
    uint32_t g = context->H[6];
    uint32_t h = context->H[7];
    uint32_t t1, t2;

    for (int k=0 ; k<64 ; k++)
    {
        t1 = h + P32_2(e) + CH(e, f, g) + SHA2x32_K[k] + X[k];
        t2 = P32_1(a) + MAJ(a, b, c);
        h = g; g = f; f = e; e = d + t1; d = c; c = b; b = a; a = t1+t2;
    }

    context->H[0] += a;
    context->H[1] += b;
    context->H[2] += c;
    context->H[3] += d;
    context->H[4] += e;
    context->H[5] += f;
    context->H[6] += g;
    context->H[7] += h;
}

void SHA2x64ProcessBlock(SHA2x64Context* context, const unsigned char* block)
{
    context->bitLen += 1024;

    uint64_t X[80];
    for (int k=0 ; k<16 ; k++)
    {
        ENCODE64BI(X[k], block[k*8], block[k*8+1], block[k*8+2], block[k*8+3], block[k*8+4], block[k*8+5], block[k*8+6], block[k*8+7])
    }
    for (int k=16 ; k<80 ; k++)
    {
        X[k] = P64_4(X[k-2]) + X[k-7] + P64_3(X[k-15]) + X[k-16];
    }

    uint64_t a = context->H[0];
    uint64_t b = context->H[1];
    uint64_t c = context->H[2];
    uint64_t d = context->H[3];
    uint64_t e = context->H[4];
    uint64_t f = context->H[5];
    uint64_t g = context->H[6];
    uint64_t h = context->H[7];
    uint64_t t1, t2;

    for (int k=0 ; k<80 ; k++)
    {
        t1 = h + P64_2(e) + CH(e, f, g) + SHA2x64_K[k] + X[k];
        t2 = P64_1(a) + MAJ(a, b, c);
        h = g; g = f; f = e; e = d + t1; d = c; c = b; b = a; a = t1+t2;
    }

    context->H[0] += a;
    context->H[1] += b;
    context->H[2] += c;
    context->H[3] += d;
    context->H[4] += e;
    context->H[5] += f;
    context->H[6] += g;
    context->H[7] += h;
}

void SHA2x32ProcessLastBlock(SHA2x32Context* context, const unsigned char* lastBlock, unsigned int len)
{
    unsigned char block[64];
    if (len == 64)
    {
        SHA2x32ProcessBlock(context, lastBlock);
        len = 0;
    }
    context->bitLen += len*8;

    for (unsigned int k=0 ; k<len ; k++)
    {
        block[k] = lastBlock[k];
    }
    block[len] = 0b10000000; // Byte padding
    if (len >= 56)
    {
        for (unsigned int k=len+1 ; k<64 ; k++)
        {
            block[k] = 0;
        }
        SHA2x32ProcessBlock(context, block);
        context->bitLen -= 512;
        for (int k=0 ; k<56 ; k++)
        {
            block[k] = 0;
        }
    }
    else
    {
        for (unsigned int k=len+1 ; k<56 ; k++)
        {
            block[k] = 0;
        }
    }
    DECODE64BI(block[56], block[57], block[58], block[59], block[60], block[61], block[62], block[63], ((uint64_t) context->bitLen))
    SHA2x32ProcessBlock(context, block);
}

void SHA2x64ProcessLastBlock(SHA2x64Context* context, const unsigned char* lastBlock, unsigned int len)
{
    unsigned char block[128];
    if (len == 128)
    {
        SHA2x64ProcessBlock(context, lastBlock);
        len = 0;
    }
    context->bitLen += len*8;

    for (unsigned int k=0 ; k<len ; k++)
    {
        block[k] = lastBlock[k];
    }
    block[len] = 0b10000000; // Byte padding
    if (len >= 112)
    {
        for (unsigned int k=len+1 ; k<128 ; k++)
        {
            block[k] = 0;
        }
        SHA2x64ProcessBlock(context, block);
        context->bitLen -= 1024;
        for (int k=0 ; k<112 ; k++)
        {
            block[k] = 0;
        }
    }
    else
    {
        for (unsigned int k=len+1 ; k<112 ; k++)
        {
            block[k] = 0;
        }
    }
    for (int k=112 ; k<120 ; k++) block[k] = 0;
    DECODE64BI(block[120], block[121], block[122], block[123], block[124], block[125], block[126], block[127], ((uint64_t) context->bitLen))
    // Normally we have to decode a 128 bits len but 2**64 is big enough.
    SHA2x64ProcessBlock(context, block);
}

void SHA2x32GetDigest(unsigned char* digest, const SHA2x32Context* context)
{
    for (int k=0 ; k<8 ; k++)
    {
        DECODE32BI(digest[k*4], digest[k*4+1], digest[k*4+2], digest[k*4+3], context->H[k])
    }
}

void SHA2x64GetDigest(unsigned char* digest, const SHA2x64Context* context)
{
    for (int k=0 ; k<8 ; k++)
    {
        DECODE64BI(digest[k*8], digest[k*8+1], digest[k*8+2], digest[k*8+3], digest[k*8+4], digest[k*8+5], digest[k*8+6], digest[k*8+7], context->H[k])
    }
}